...
// 引数は対象の座標x1, y1 と対象の大きさ w, h
func (s *Sprite) IsCollide(x1, y1, w1, h1 int) {
	img := s.currentImage()
	x := s.Position.X
	y := s.Position.Y
	w, h := img.Size()
	...
	if (x + w > x1) {
		fmt.Println("プレイヤーの右端と、対象の左端が衝突")
	}
	if (x < x1 + w1) {
		fmt.Println("プレイヤーの左端と、対象の右端が衝突")
	}
	if (y + h > y1) {
		fmt.Println("プレイヤーの下限と、対象の上限が衝突")
	}
	if (y < y1 + h1) {
		fmt.Println("プレイヤーの上限と、対象の下限が衝突")
	}
}
